...
contract KickStarter {
    ...
    mapping(uint => KickCampaign) public campaigns;
    ...
    function start(uint _goal, uint _endsAt) external {
        ...
        Campaign newCampign = new Campaign(_endsAt, _goal, msg.sender, currentCampaign);

        campaigns[currentCampaign] = KickCampaign({
            targetContract: newCampign, 
            claimed: false
        });
    ...
    function onClaimed(uint id) external {
        KickCampaign storage targetCampaign = campaigns[id];
